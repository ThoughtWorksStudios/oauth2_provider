  ...
  module Provider
    class OAuthClientsControllerTest < ActionController::TestCase
      ...
      def test_create_creates_new_oauth_client
        post :create, :o_auth_client => {:name => 'my application', :redirect_uri => 'http://api.example.com/cb'}
      ...
      def test_should_not_create_oauth_client_without_name_or_redirect_uri
        post :create, :o_auth_client => {:name => 'my application'}
        assert_equal 0, OAuthClient.count
        post :create, :o_auth_client => {:redirect_uri => 'http://api.example.com/cb'}
        assert_equal 0, OAuthClient.count
      end
      ...
      def test_update_writes_new_attributes
        ...
        post :update, :id => client.id, :o_auth_client => {:name => 'new name', :redirect_uri => 'http://new.example.com/cb'}
        client.reload
      ...
      def test_edit_should_populate_form_with_correct_client_info
        ...
        assert_select "form" do
          assert_select "input[type='text'][name='o_auth_client[name]'][value='#{client.name}']"
          assert_select "input[type='text'][name='o_auth_client[redirect_uri]'][value='#{client.redirect_uri}']"
        end
      end
