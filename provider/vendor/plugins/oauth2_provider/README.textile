...
class ApplicationController < ActionController::Base
  ...
  # your existing before_filter to check if a user has logged in before attempting to access a resource
  before_filter :login_required
...
The names of the method "login_required_with_oauth" and the actions within should match your before_filter to log in to the app.  For example, if your pre-existing login method were called "authenticate" instead of "login_required", the method would then be called "authenticate_with_oauth" and the alias_method_chain would be "alias_method_chain:authenticate, :oauth"

Also, you will need to replace "YOUR_APPLICATION_SESSION_KEY" with your session id.  
...
* Your browser is redirected to your redirect URI (hint: use example.com for this test) and you should now see your authorization code.  Save this code, you will need it for the next step.
